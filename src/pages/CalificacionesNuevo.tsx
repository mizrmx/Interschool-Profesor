import { FunctionComponent } from "react";
import styles from "./CalificacionesNuevo.module.css";

const CalificacionesNuevo: FunctionComponent = () => {
  return (
    <div className={styles.calificacionesNuevo}>
      <div className={styles.component1}>
        <div className={styles.sideMenu}>
          <div className={styles.placeholder} />
          <div className={styles.name}>
            <div className={styles.mayraZ}>Mayra Z.</div>
            <div className={styles.admin}>Admin</div>
          </div>
          <div className={styles.contrasea}>
            <div className={styles.bg} />
            <img className={styles.iconKey} alt="" src="/-icon-key.svg" />
          </div>
          <div className={styles.disponibilidad}>
            <div className={styles.bg} />
            <img
              className={styles.iconCalendarCheck}
              alt=""
              src="/-icon-calendar-check.svg"
            />
          </div>
          <div className={styles.inbox}>
            <div className={styles.bg} />
            <img className={styles.iconInbox} alt="" src="/-icon-inbox.svg" />
          </div>
        </div>
        <div className={styles.sidebar}>
          <div className={styles.background} />
          <div className={styles.footer}>
            <b className={styles.scolarpanel}>Scolarpanel</b>
            <div className={styles.rectangleParent}>
              <div className={styles.groupChild} />
              <b className={styles.i}>I</b>
            </div>
          </div>
          <div className={styles.list}>
            <div className={styles.dashboard}>
              <img className={styles.homeIcon} alt="" src="/home.svg" />
              <div className={styles.email}>Inicio</div>
            </div>
            <div className={styles.students}>
              <img className={styles.homeIcon} alt="" src="/student.svg" />
              <div className={styles.email}>Calificaciones</div>
            </div>
            <div className={styles.students}>
              <img className={styles.homeIcon} alt="" src="/teacher.svg" />
              <div className={styles.email}>Actas Calificaciones</div>
            </div>
            <div className={styles.students}>
              <img className={styles.homeIcon} alt="" src="/calendar.svg" />
              <div className={styles.email}>Reporte Aprobación</div>
            </div>
            <div className={styles.students}>
              <img className={styles.homeIcon} alt="" src="/finance.svg" />
              <div className={styles.email}>Actas Submaterias</div>
            </div>
            <div className={styles.food}>
              <img
                className={styles.iconPdfFile}
                alt=""
                src="/-icon-pdf-file.svg"
              />
              <div className={styles.email}>Publicaciones</div>
            </div>
            <div className={styles.students}>
              <img className={styles.homeIcon} alt="" src="/user.svg" />
              <div className={styles.email}>Consulta Publicaciones</div>
            </div>
            <div className={styles.students}>
              <img className={styles.homeIcon} alt="" src="/chat.svg" />
              <div className={styles.email}>Reactivos</div>
            </div>
            <div className={styles.students}>
              <img className={styles.homeIcon} alt="" src="/activity.svg" />
              <div className={styles.email}>Actos Finales</div>
            </div>
            <div className={styles.students}>
              <img className={styles.homeIcon} alt="" src="/activity.svg" />
              <div className={styles.email}>Aspectos Evaluación</div>
            </div>
            <div className={styles.students}>
              <img className={styles.homeIcon} alt="" src="/activity.svg" />
              <div className={styles.email}>Evaluación Continua</div>
            </div>
            <div className={styles.students}>
              <img className={styles.homeIcon} alt="" src="/activity.svg" />
              <div className={styles.email}>Calificación Evaluación</div>
            </div>
            <div className={styles.students}>
              <img className={styles.homeIcon} alt="" src="/activity.svg" />
              <div className={styles.email}>Asistencia</div>
            </div>
          </div>
          <div className={styles.logo}>
            <b className={styles.escuelaBenitoJuarez}>Escuela Benito Juarez</b>
            <div className={styles.rectangleGroup}>
              <div className={styles.groupChild} />
              <b className={styles.e}>E</b>
            </div>
          </div>
        </div>
      </div>
      <div className={styles.menuTable}>
        <div className={styles.botonGuardar}>
          <div className={styles.botonGuardarChild} />
          <div className={styles.guardar}>Guardar</div>
        </div>
        <div className={styles.menuTableChild} />
        <div className={styles.div}>#</div>
        <div className={styles.id}>ID</div>
        <div className={styles.nombre}>Nombre</div>
        <div className={styles.extra1}>Extra 1</div>
        <div className={styles.extra2}>Extra 2</div>
        <div className={styles.extra3}>Extra 3</div>
        <div className={styles.calificacin}>Calificación</div>
        <div className={styles.faltas}>Faltas</div>
        <div className={styles.disciplina}>Disciplina</div>
        <div className={styles.acciones}>Acciones</div>
      </div>
      <div className={styles.frameParent}>
        <div className={styles.parent}>
          <div className={styles.div1}>1</div>
          <div className={styles.mat1}>MAT 1</div>
          <div className={styles.emilianoRivas}>Emiliano Rivas</div>
          <div className={styles.div2}>7.5</div>
          <div className={styles.div3}>7.5</div>
          <div className={styles.div4}>7.5</div>
          <div className={styles.div5}>7.5</div>
          <div className={styles.div6}>5.0</div>
          <div className={styles.div7}>10</div>
          <img className={styles.vectorIcon} alt="" src="/vector12.svg" />
          <img className={styles.vectorIcon1} alt="" src="/vector13.svg" />
        </div>
        <div className={styles.group}>
          <div className={styles.div8}>2</div>
          <div className={styles.mat1}>MAT 1</div>
          <div className={styles.emilianoRivas1}>Emiliano Rivas</div>
          <div className={styles.div9}>7.5</div>
          <div className={styles.div10}>7.5</div>
          <div className={styles.div11}>7.5</div>
          <div className={styles.div12}>7.5</div>
          <div className={styles.div13}>5.0</div>
          <div className={styles.div14}>10</div>
          <img className={styles.vectorIcon2} alt="" src="/vector12.svg" />
          <img className={styles.vectorIcon3} alt="" src="/vector13.svg" />
        </div>
        <div className={styles.container}>
          <div className={styles.div15}>3</div>
          <div className={styles.mat12}>MAT 1</div>
          <div className={styles.emilianoRivas2}>Emiliano Rivas</div>
          <div className={styles.div16}>7.5</div>
          <div className={styles.div10}>7.5</div>
          <div className={styles.div18}>7.5</div>
          <div className={styles.div12}>7.5</div>
          <div className={styles.div20}>5.0</div>
          <div className={styles.div14}>10</div>
          <img className={styles.vectorIcon4} alt="" src="/vector12.svg" />
          <img className={styles.vectorIcon5} alt="" src="/vector13.svg" />
        </div>
        <div className={styles.frameDiv}>
          <div className={styles.div22}>4</div>
          <div className={styles.mat13}>MAT 1</div>
          <div className={styles.emilianoRivas3}>Emiliano Rivas</div>
          <div className={styles.div16}>7.5</div>
          <div className={styles.div24}>7.5</div>
          <div className={styles.div25}>7.5</div>
          <div className={styles.div26}>7.5</div>
          <div className={styles.div27}>5.0</div>
          <div className={styles.div28}>10</div>
          <img className={styles.vectorIcon6} alt="" src="/vector12.svg" />
          <img className={styles.vectorIcon7} alt="" src="/vector13.svg" />
        </div>
        <div className={styles.parent1}>
          <div className={styles.div29}>5</div>
          <div className={styles.mat12}>MAT 1</div>
          <div className={styles.emilianoRivas3}>Emiliano Rivas</div>
          <div className={styles.div30}>7.5</div>
          <div className={styles.div31}>7.5</div>
          <div className={styles.div32}>7.5</div>
          <div className={styles.div33}>7.5</div>
          <div className={styles.div34}>5.0</div>
          <div className={styles.div35}>10</div>
          <img className={styles.vectorIcon8} alt="" src="/vector12.svg" />
          <img className={styles.vectorIcon9} alt="" src="/vector13.svg" />
        </div>
        <div className={styles.parent2}>
          <div className={styles.div36}>6</div>
          <div className={styles.mat15}>MAT 1</div>
          <div className={styles.emilianoRivas5}>Emiliano Rivas</div>
          <div className={styles.div30}>7.5</div>
          <div className={styles.div31}>7.5</div>
          <div className={styles.div39}>7.5</div>
          <div className={styles.div40}>7.5</div>
          <div className={styles.div41}>5.0</div>
          <div className={styles.div35}>10</div>
          <img className={styles.vectorIcon10} alt="" src="/vector12.svg" />
          <img className={styles.vectorIcon11} alt="" src="/vector13.svg" />
        </div>
        <div className={styles.parent3}>
          <div className={styles.div43}>7</div>
          <div className={styles.mat16}>MAT 1</div>
          <div className={styles.emilianoRivas6}>Emiliano Rivas</div>
          <div className={styles.div30}>7.5</div>
          <div className={styles.div45}>7.5</div>
          <div className={styles.div39}>7.5</div>
          <div className={styles.div47}>7.5</div>
          <div className={styles.div48}>5.0</div>
          <div className={styles.div35}>10</div>
          <img className={styles.vectorIcon12} alt="" src="/vector12.svg" />
          <img className={styles.vectorIcon13} alt="" src="/vector13.svg" />
        </div>
        <div className={styles.parent4}>
          <div className={styles.div43}>8</div>
          <div className={styles.mat16}>MAT 1</div>
          <div className={styles.emilianoRivas6}>Emiliano Rivas</div>
          <div className={styles.div30}>7.5</div>
          <div className={styles.div45}>7.5</div>
          <div className={styles.div39}>7.5</div>
          <div className={styles.div47}>7.5</div>
          <div className={styles.div48}>5.0</div>
          <div className={styles.div35}>10</div>
          <img className={styles.vectorIcon12} alt="" src="/vector12.svg" />
          <img className={styles.vectorIcon13} alt="" src="/vector13.svg" />
        </div>
        <div className={styles.parent5}>
          <div className={styles.div43}>9</div>
          <div className={styles.mat16}>MAT 1</div>
          <div className={styles.emilianoRivas6}>Emiliano Rivas</div>
          <div className={styles.div30}>7.5</div>
          <div className={styles.div45}>7.5</div>
          <div className={styles.div39}>7.5</div>
          <div className={styles.div47}>7.5</div>
          <div className={styles.div48}>5.0</div>
          <div className={styles.div35}>10</div>
          <img className={styles.vectorIcon12} alt="" src="/vector12.svg" />
          <img className={styles.vectorIcon13} alt="" src="/vector13.svg" />
        </div>
        <div className={styles.parent6}>
          <div className={styles.div64}>10</div>
          <div className={styles.mat16}>MAT 1</div>
          <div className={styles.emilianoRivas6}>Emiliano Rivas</div>
          <div className={styles.div30}>7.5</div>
          <div className={styles.div45}>7.5</div>
          <div className={styles.div39}>7.5</div>
          <div className={styles.div47}>7.5</div>
          <div className={styles.div48}>5.0</div>
          <div className={styles.div35}>10</div>
          <img className={styles.vectorIcon12} alt="" src="/vector12.svg" />
          <img className={styles.vectorIcon13} alt="" src="/vector13.svg" />
        </div>
        <div className={styles.parent7}>
          <div className={styles.div43}>11</div>
          <div className={styles.mat16}>MAT 1</div>
          <div className={styles.emilianoRivas6}>Emiliano Rivas</div>
          <div className={styles.div30}>7.5</div>
          <div className={styles.div45}>7.5</div>
          <div className={styles.div39}>7.5</div>
          <div className={styles.div47}>7.5</div>
          <div className={styles.div48}>5.0</div>
          <div className={styles.div35}>10</div>
          <img className={styles.vectorIcon12} alt="" src="/vector12.svg" />
          <img className={styles.vectorIcon13} alt="" src="/vector13.svg" />
        </div>
        <div className={styles.parent8}>
          <div className={styles.div78}>12</div>
          <div className={styles.mat16}>MAT 1</div>
          <div className={styles.emilianoRivas6}>Emiliano Rivas</div>
          <div className={styles.div30}>7.5</div>
          <div className={styles.div45}>7.5</div>
          <div className={styles.div39}>7.5</div>
          <div className={styles.div47}>7.5</div>
          <div className={styles.div48}>5.0</div>
          <div className={styles.div35}>10</div>
          <img className={styles.vectorIcon12} alt="" src="/vector12.svg" />
          <img className={styles.vectorIcon13} alt="" src="/vector13.svg" />
        </div>
      </div>
      <div className={styles.education}>
        <div className={styles.bg3} />
        <div className={styles.datosConsulta}>
          <div className={styles.boton}>
            <div className={styles.botonChild} />
            <div className={styles.mostrar}>Mostrar</div>
          </div>
          <div className={styles.periodo}>
            <div className={styles.periodo1}>Periodo</div>
            <div className={styles.input}>
              <div className={styles.inputChild} />
              <div className={styles.div85}>1</div>
              <img
                className={styles.materialSymbolskeyboardArroIcon}
                alt=""
                src="/materialsymbolskeyboardarrowdownrounded.svg"
              />
            </div>
          </div>
          <div className={styles.materia}>
            <div className={styles.periodo1}>Materia</div>
            <div className={styles.input1}>
              <div className={styles.inputItem} />
              <div className={styles.seleccione}>Seleccione</div>
              <img
                className={styles.materialSymbolskeyboardArroIcon1}
                alt=""
                src="/materialsymbolskeyboardarrowdownrounded.svg"
              />
            </div>
          </div>
          <div className={styles.grupo}>
            <div className={styles.periodo1}>Grupo</div>
            <div className={styles.input}>
              <div className={styles.inputChild} />
              <div className={styles.div85}>1</div>
              <img
                className={styles.materialSymbolskeyboardArroIcon}
                alt=""
                src="/materialsymbolskeyboardarrowdownrounded.svg"
              />
            </div>
          </div>
          <div className={styles.nivel}>
            <div className={styles.periodo1}>Nivel</div>
            <div className={styles.input1}>
              <div className={styles.inputItem} />
              <div className={styles.seleccione}>Seleccione</div>
              <img
                className={styles.materialSymbolskeyboardArroIcon1}
                alt=""
                src="/materialsymbolskeyboardarrowdownrounded.svg"
              />
            </div>
          </div>
          <div className={styles.ciclo}>
            <div className={styles.periodo1}>Ciclo</div>
            <div className={styles.input4}>
              <div className={styles.inputChild1} />
              <div className={styles.div87}>2023</div>
              <img
                className={styles.materialSymbolskeyboardArroIcon4}
                alt=""
                src="/materialsymbolskeyboardarrowdownrounded.svg"
              />
            </div>
          </div>
        </div>
        <div className={styles.title}>
          <div className={styles.titleChild} />
          <b
            className={styles.calificacionesMaterias}
          >{`Calificaciones Materias `}</b>
        </div>
      </div>
    </div>
  );
};

export default CalificacionesNuevo;
